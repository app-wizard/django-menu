...
class ProductViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.category = Categories.objects.create(
            name="Nigiri Selections", slug="nigiri-selections"
        )
        cls.product = Products.objects.create(
            name="Tuna Sashimi", slug="tuna-sashimi", category=cls.category
        )
        cls.user = User.objects.create_user(
            username="test123456", password="Ttt55555"
        )  # Moved user creation here
    ...
    def test_comment_submission(self):
        ...
        response = self.client.post(
            reverse("goods:product", args=["tuna-sashimi"]), {"body": "New Comment"}
        )
...
class CommentUpdateDeleteTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Products.objects.create(
            name="TestProduct", slug="test-product", category=self.category
        )
        self.comment = Comment.objects.create(
            dish=self.product, body="Original Comment", author=self.user, approved=True
        )
    ...
    def test_comment_edit(self):
        ...
        url = reverse(
            "goods:comment_edit",
            kwargs={"product_slug": self.product.slug, "comment_id": self.comment.id},
        )
